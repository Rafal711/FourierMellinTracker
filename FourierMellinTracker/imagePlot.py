...
def plotImages1x3(pattern, searchedImg, orgSize, position, orgPattern, visibilityFlag=True, frameNr=0, similarity=0):
    squareX = position[0] - orgSize[0] // 2 - 1
    squareY = position[1] - orgSize[1] // 2 - 1
    box1 = patches.Rectangle((squareX, squareY), orgSize[0], orgSize[1],
                             linewidth=1, edgecolor='r', facecolor='none')
    box2 = copy.copy(box1)

    fig, (ax0, ax1, ax2) = plt.subplots(1, 3, figsize=(12, 7))

    ax0.imshow(orgPattern, cmap='gray', vmin=0, vmax=255)
    ax0.set_title("pattern")
